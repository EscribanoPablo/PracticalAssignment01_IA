...
namespace Steerings
{
    ...
    {
        //cada steering se encarga de generar y aplicar sus aceleraciones
        ...
        private void ApplyLinearAccelerationWithoutRigidBody ()
        {
            ...
            Vector3 velIncrement = acceleration * dt; // v = a * t
            Context.velocity += velIncrement;
            
            if (Context.clipVelocity)
                if (Context.velocity.magnitude > Context.maxSpeed)
                    Context.velocity = Context.velocity.normalized * Context.maxSpeed; //para clipear la velocidad, que no se pase del maximo;
